...
<div id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?> collections-sidebar-second clearfix"<?php print $attributes; ?>>
	...
	<div class="collections-sidebar sidebar-second sidebar-offcanvas col-xs-3 col-md-3">
        ...
        <h4>Visibility</h4>
        <?php
        // Not sure if this case should be handled ....
        if (!isset($content['group_access']))
        {
		...
		<h4>Members</h4>
		<?php print views_embed_view($members_view[0],$members_view[1],$node->nid,$og_parent_id); // Extra argument does not work :-( ?>
            <?php if($type == 'subcollection'): ?>
            <h5>Inhereted members</h5>
            <?php print views_embed_view($members_view[0],$members_view[1],$og_parent_id); ?>
            <?php endif; ?>
        <?php endif; ?>
