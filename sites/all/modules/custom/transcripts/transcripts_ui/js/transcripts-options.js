      ...
      $('[data-transcripts-role=transcript-controls]', context)
        ...
        .each(function () {
          ...
          $transcriptOptions.change(function (e) {
            var sweetSpot = $scroller.sweetSpot; // get current scroll position
            var $last = $('.playing', $transcript).filter(':last');

            //language selection - changes segment height so affects scroll position
            ...
            //transcript view selection - minimal view changes segment height so affects scroll position
            $('optgroup[data-type=views] option', this).each(function () {
              $transcript.toggleClass($(this).val(), $(this).prop('selected'));
            });

            $scroller.sweetSpot = sweetSpot; // set scroll position back to what it was
            if ($last.length == 1) {
              $scroller.startPlay($last);
            }
